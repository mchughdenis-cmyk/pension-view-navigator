...
export async function downloadTransferConfirmation(clientName: string = 'Emma Wilson') {
  ...
  const content = [
    ...
    new Paragraph({ children: [new TextRun({ text: 'Transfer Reference: ', bold: true }), new TextRun('TRF-2024-0115')], spacing: { after: 100 } }),
...
export async function downloadDrawdownConfirmation(clientName: string = 'David Thompson') {
  ...
  const content = [
    ...
    new Paragraph({ children: [new TextRun({ text: 'Reference: ', bold: true }), new TextRun('DWN-2024-0115')], spacing: { after: 100 } }),
